 ...
 * Gerado pelo plugin with-smart2-print. Usa PlugPag.printFromFile com PNG
 * gerado do texto.
 */
public class Smart2PrintModule extends ReactContextBaseJavaModule {

    // Largura útil da bobina 58mm. Render 2x (traço nítido) + downscale com filtro
    // = mais qualidade.
    ...
    public void print(String text, Promise promise) {
        try {
            ...
            PlugPagPrinterData printerData = new PlugPagPrinterData(filePath, 2, 0);
    ...
    private String textToPrintFile(String text) {
        ...
        // Desenho em 2x com texto nítido (sem antialiasing); depois downscale com
        // filtro
        ...
        int contentHeight = pad * 2 + (lines.length > 0 ? headerLineHeight : 0)
                + (lines.length > 1 ? (lines.length - 1) * lineHeight : 0);
        ...
        // Desenho duplicado com 1px de deslocamento = texto mais grosso/escuro no
        // térmico
        ...
        // O PlugPag TerminalLib roda em outro processo e não acessa o cache privado do
        // app.
