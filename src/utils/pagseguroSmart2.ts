...
/** Verifica se o erro é de timeout (nosso ou do módulo nativo/SDK). */
function isTimeoutError(e: unknown): boolean {
  ...
  return (
    ...
    msg.includes('Tempo de resposta excedido') ||
    (msg.includes('excedido') && lower.includes('tempo')) ||
    msg.includes('ESGOTADO O TEMPO MAXIMO') ||
    msg.includes('TEMPO MAXIMO ESTIPULADO') ||
    lower.includes('timeout')
  );
}

/** Uma única tentativa de pagamento (race com timeout JS). */
 ...
 * O SDK/módulo nativo tem timeout próprio (~10–12 s). Em caso de "Tempo de resposta excedido"
 * é feita uma segunda tentativa automática antes de falhar.
 *
...
export async function payWithSmart2(
  ...
  const runAttempt = (): Promise<PagSeguroPayResponse> =>
    attemptPay(amountInCents, reference, paymentType, installments, timeoutMs, startTime);
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      const elapsed = Date.now() - startTime;
      const err = e as { code?: string; message?: string };
      console.error(`[PagSeguro] Erro após ${elapsed}ms (tentativa ${attempt}/2):`, err);

      const errorMessage = err?.message ?? String(e);

      // Timeout (nosso ou do nativo): na primeira tentativa, retry automático
      if (isTimeoutError(e)) {
        if (attempt === 1) {
          console.log(`[PagSeguro] Timeout na 1ª tentativa; iniciando 2ª tentativa. Aproxime o cartão.`);
          continue;
        }
        return {
          success: false,
          code: 'TIMEOUT',
          message: 'Tempo máximo estipulado para a operação expirou. Aproxime o cartão em até ~10 s e tente novamente.',
        };
      }
